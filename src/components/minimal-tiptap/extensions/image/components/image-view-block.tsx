...
export const ImageViewBlock: React.FC<NodeViewProps> = ({ editor, node, selected, updateAttributes }) => {
  ...
  return (
    <NodeViewWrapper ref={containerRef} data-drag-handle className="relative text-center leading-none">
      ...
      >
        <div
          className={cn('relative flex h-full cursor-default flex-col items-center gap-2 rounded', editor.isEditable && {
        ...
        >
          ...
          {editor.isEditable &&!isResizing && !imageState.error && !imageState.isServerUploading && (
            <ImageActions
